...
fn part2(asteroids_pos : &Vec<Pos>, station_pos : &Pos) {
  ...
  let mut map : BTreeMap<OrderedFloat<f32>, Vec<Asteroid>> = BTreeMap::new();
  for asteroid_pos in asteroids_pos {
    ...
    let angle_part1 = if rad_angle > 0.0 { rad_angle } else { (2.0 * std::f32::consts::PI + rad_angle) };
    let angle = angle_part1 * 360.0 / (2.0 * std::f32::consts::PI);
    map.entry(OrderedFloat(angle)).or_default().push(Asteroid{
    ...
    for (_, asteroids) in &mut map.iter_mut().rev() {
      match asteroids.pop() {
        Some(asteroid) => {
          ...
          found_asteroid = true;
          println!("Annihilate: ({},{}) distance {}", asteroid.pos.x, asteroid.pos.y, asteroid.distance);
        },
        None => continue,
      }
    }
